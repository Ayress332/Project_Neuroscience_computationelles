...
if __name__ == '__main__':
    ...
    j2 = ML.J(3, 0.27)
    det2 = linalg.det(j2)
    vp2 = linalg.eigvals(j2)
    tr.append(sum(vp2))
    
    #Equilibre 2###
    j3 = ML.J(-60,0.01)
    det3 = linalg.det(j3)
    vp3 = linalg.eigvals(j3)
    tr.append(sum(vp3))
    
    vp=[vp1, vp2, vp3] #valeurs propres
    det=[det1, det2, det3] #déterminants
    
    for i in range (len(vp)):
        #----
        if all([vpi > 0 for vpi in vp[i]]): print("minimum, instable")
        elif all([vpi < 0 for vpi in vp[i]]): print("maximum, stable")
        else : print("point selle, instable")
        #----
        if tr[i]**2 < 4*det[i]: print(f"j {i} est une spirale")
        else: print(f"j {i} est indéterminé (un neud ?)")
    ...
    #print(ML.V_intersept(ML.w_inf, ML.V_nullcline))
    #wp=(-0.04*w + 0.02*tanh(v/30 - 3/5) + 0.02)*cosh(v/60 - 1/30)
